...
fn main() -> anyhow::Result<()> {
    eprintln!("Starting");
    match install() {
        Ok(_) => eprintln!("Successfully installed service!"),
        Err(_) => eprintln!("Service already installed"),
    };
    geph_service::run()?;
    eprintln!("Ran service");
...
fn install() -> windows_service::Result<()> {
    ...
    let service_binary_path = ::std::env::current_exe()
        .expect("Error retrieving binary path");

    let service_info = ServiceInfo {
        name: OsString::from("geph_service"),
        display_name: OsString::from("Geph service"),
...
mod geph_service {
    ...
    const SERVICE_NAME: &str = "geph_service";
